...
int initGameModeZero(LANG *Lang) {
    printf(Lang->ola);// prints out name lab and time
    ...
    while(true) { 		// main game while loop
        printf(Lang->olauser); // game greeting
        printf(Lang->primeiraguess); 	// computers first guess
        ...
        while(true) { 		// while loop for game logic
            ...
            if (UI == -1) { 			// if statement for user input -1, logic if guess is to low
                ...
                printf(Lang->mtbaixa);
                printf(Lang->proximaguess);
                printf("%d",CG);
                printf(Lang->comofui);
            } 						// prints out next current guess
            if (UI == 0) {
                printf("%s%d",Lang->tentativas,counter);
                printf("%s%d",Lang->unumero,CG);
                break; 				// breaks the logic game loop
            }
        }
        printf(Lang->jogardnv); // prompts user input
